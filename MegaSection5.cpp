...
IMPLEMENT_DYNAMIC(CMegaSection5, CDialog)

CMegaSection5::CMegaSection5(CWnd* pParent /*=NULL*/)
	: CDialog(CMegaSection5::IDD, pParent)
{
/*
	m_sect5_hc = 3072;
	m_sect5_bc = 3072;
...
CMegaSection5::~CMegaSection5()
{
}
BOOL CMegaSection5::PreTranslateMessage(MSG* pMsg)   
{  
...
void CMegaSection5::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	DDX_Text(pDX, IDC_S5_HC, m_sect5_hc);
	DDX_Text(pDX, IDC_S5_BC, m_sect5_bc);
	DDX_Control(pDX, IDC_STRU_CLASS, m_stru_class);
	DDX_Control(pDX, IDIDC_EXP_CLASS, m_exp_class);
	...
	DDX_Text(pDX, IDC_BAR_MP_NEW5, m_bar_mp_new);
	...
	DDX_Text(pDX, IDC_TRNBAR_MP_NEW5, m_tranbar_mp);
	DDX_Control(pDX, IDC_STEEL_FY4, m_steel_code);
	DDX_Text(pDX, IDC_tranratio_yy_5, m_tranratio_yy);
	DDX_Text(pDX, IDC_tranratio_zz_5, m_tranratio_zz);
}


BEGIN_MESSAGE_MAP(CMegaSection5, CDialog)
	ON_BN_CLICKED(IDC_USER_DEF5, &CMegaSection5::OnBnClickedUserDef5)
	ON_BN_CLICKED(IDC_CHOOSE_PROFILE, &CMegaSection5::OnBnClickedChooseProfile)
	ON_BN_CLICKED(IDOK, &CMegaSection5::OnBnClickedOk)
	ON_CBN_SELCHANGE(IDC_BAR_D, &CMegaSection5::OnCbnSelchangeBarD)	
	ON_CBN_SELCHANGE(IDC_CON_CLASS, &CMegaSection5::OnCbnSelchangeConClass)
	ON_CBN_SELCHANGE(IDC_STEEL_FY, &CMegaSection5::OnCbnSelchangeSteelFy)
	ON_CBN_SELCHANGE(IDC_STRU_CLASS, &CMegaSection5::OnCbnSelchangeStruClass)
	ON_CBN_SELCHANGE(IDC_EXP_CLASS, &CMegaSection5::OnCbnSelchangeExpClass)
	ON_CBN_SELCHANGE(IDC_STEEL_FY4, &CMegaSection5::OnCbnSelchangeSteelFy4)
	ON_EN_KILLFOCUS(IDC_S5_HC, &CMegaSection5::OnEnKillfocusS5Hc)
	ON_EN_KILLFOCUS(IDC_S5_BC, &CMegaSection5::OnEnKillfocusS5Bc)
	ON_EN_KILLFOCUS(IDC_BAR_Y, &CMegaSection5::OnEnKillfocusBarY)
	ON_EN_KILLFOCUS(IDC_BAR_X, &CMegaSection5::OnEnKillfocusBarX)
	ON_EN_KILLFOCUS(IDC_P_C, &CMegaSection5::OnEnKillfocusPC)
	ON_EN_KILLFOCUS(IDC_P_D, &CMegaSection5::OnEnKillfocusPD)
	ON_WM_PAINT()
	ON_EN_KILLFOCUS(IDC_EDIT_CSZ, &CMegaSection5::OnEnKillfocusEditCsz)
	ON_EN_KILLFOCUS(IDC_EDIT_CSY, &CMegaSection5::OnEnKillfocusEditCsy)
END_MESSAGE_MAP()


// CMegaSection5 消息处理程序


BOOL CMegaSection5::OnInitDialog()
{
	CDialog::OnInitDialog();

	// TODO:  在此添加额外的初始化
	if(GlobalPara::ExpertMode==true){
		GetDlgItem(IDC_USER_DEF5)->ShowWindow(SW_SHOW);
		m_con_class.AddString("Custom");		//自定义本构
	}
	else
		GetDlgItem(IDC_USER_DEF5)->ShowWindow(SW_HIDE);
	...
	m_sect5_hc = GlobalPara::m_Sect_in.Sect_h;
	m_sect5_bc = GlobalPara::m_Sect_in.Sect_b;
...
void CMegaSection5::OnBnClickedUserDef5()
{
	// TODO: 在此添加控件通知处理程序代码
	CUserdefcross pdlg;
	int ik = pdlg.DoModal();
...
void CMegaSection5::OnBnClickedChooseProfile()
{
	// TODO: 在此添加控件通知处理程序代码
// 	int iType=7,IDel=0;
...
void CMegaSection5::SetGlobalPara(void)
{
	UpdateData();
	GlobalPara::m_Sect_in.HSect_x = m_sect5_bc*0.5-m_profile_d;
	GlobalPara::m_Sect_in.HSect_y = m_sect5_hc*0.5-m_profile_c;
	...
	GlobalPara::m_Sect_in.Sect_b = m_sect5_bc;
	GlobalPara::m_Sect_in.Sect_h=m_sect5_hc;
	...
	float interval1 = (m_sect5_hc-2*m_cnom)/(m_no_x-1);
	float interval2 = (m_sect5_bc-2*m_cnom)/(m_no_y-1);
...
void CMegaSection5::OnBnClickedOk()
{
	// TODO: 在此添加控件通知处理程序代码
	//d~bc~x,c~hc~y
	UpdateData();
...
//void CMegaSection5::OnCbnSelchangeNoY()
//{
//	UpdateData();
//	// TODO: 在此添加控件通知处理程序代码
//	int no[5] = {2,3,4,5,6};
...
//
//
//void CMegaSection5::OnCbnSelchangeNoX()
//{
//	// TODO: 在此添加控件通知处理程序代码
//	UpdateData();
//	int no[5] = {2,3,4,5,6};
...
void CMegaSection5::OnCbnSelchangeBarD()
{
	// TODO: 在此添加控件通知处理程序代码
	UpdateData();
...
void CMegaSection5::OnCbnSelchangeConClass()
{
	// TODO: 在此添加控件通知处理程序代码
	UpdateData();
	CString conmat;
...
void CMegaSection5::OnCbnSelchangeSteelFy()
{
	// TODO: 在此添加控件通知处理程序代码
	UpdateData();
...
void CMegaSection5::OnCbnSelchangeStruClass()
{
	// TODO: 在此添加控件通知处理程序代码
	UpdateData();
...
void CMegaSection5::OnCbnSelchangeExpClass()
{
	// TODO: 在此添加控件通知处理程序代码
	UpdateData();
...
int CMegaSection5::SetBarMp(int BarMp)
{
...
int CMegaSection5::SetBarD(int BarD)
{
	//	int bard[10]={8,10,12,14,16,20,25,28,32,40};
	for(int i=0;i<10;i++){
...
void CMegaSection5::OnCbnSelchangeSteelFy4()
{
	// TODO: 在此添加控件通知处理程序代码
	if(update)
		UpdateData();
...
void CMegaSection5::OnEnKillfocusS5Hc()
{
	// TODO: 在此添加控件通知处理程序代码
	UpdateData();
	m_no_y=int(m_sect5_hc/150.+1+0.99);
	UpdateData(FALSE);
...
void CMegaSection5::OnEnKillfocusS5Bc()
{
	// TODO: 在此添加控件通知处理程序代码
	UpdateData();	
	m_no_x=int(m_sect5_bc/150.+1+0.99);
	UpdateData(FALSE);
...
void CMegaSection5::OnEnKillfocusBarY()
{
	// TODO: 在此添加控件通知处理程序代码
	SetGlobalPara();
	DrawSectOnPic();
...
void CMegaSection5::OnEnKillfocusBarX()
{
	// TODO: 在此添加控件通知处理程序代码
	SetGlobalPara();
	DrawSectOnPic();
...
void CMegaSection5::OnEnKillfocusPC()
{
	// TODO: 在此添加控件通知处理程序代码
	SetGlobalPara();
	DrawSectOnPic();
...
void CMegaSection5::OnEnKillfocusPD()
{
	// TODO: 在此添加控件通知处理程序代码
	SetGlobalPara();
	DrawSectOnPic();
...
void CMegaSection5::DrawSectOnPic()
{
	CPaintDC dc(this);
	UpdateData();
	//	CPaintDC dc(this); // device context for painting
...
void CMegaSection5::OnEnKillfocusEditCsz()
{
	// TODO: 在此添加控件通知处理程序代码
	SetGlobalPara();
	DrawSectOnPic();
...
void CMegaSection5::OnEnKillfocusEditCsy()
{
	// TODO: 在此添加控件通知处理程序代码
	SetGlobalPara();
	DrawSectOnPic();
